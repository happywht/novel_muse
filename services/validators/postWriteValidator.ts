/**
 * Post-write rule-based validator (ported from InkOS).
 *
...
export interface PostWriteOptions {
  /** Genre-specific fatigue words (each word max 1 per chapter) */
  fatigueWords?: string[];
  /** Book-level prohibition phrases (2-30 chars, exact match) */
  prohibitions?: string[];
}

/**
 * Validate generated content against deterministic rules.
 * Returns array of violations. Empty array = all clear.
 */
export function validatePostWrite(
  ...
  const prohibitions = options?.prohibitions ?? [];
...
/** Format violations as a human-readable summary string */
