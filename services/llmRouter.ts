...
export type LLMTaskType =
  ...
  | 'generateSingleCharacter' // NEW: Single character generation
  | 'generateCharacterImage' // NEW: Character image generation
  | 'batchGenerateWorldSettingsByCategory' // NEW: Batch world settings generation
  | 'consolidateMemory'; // NEW: Memory consolidation
...
export const getProviderForTask = (task: LLMTaskType): Provider => {
  switch (task) {
    ...
    case 'generateSingleCharacter': // 单个角色生成 (Flash可胜任)
    case 'generateCharacterImage': // 角色图像生成提示
    case 'batchGenerateWorldSettingsByCategory': // 批量世界设定生成
    case 'consolidateMemory': // 记忆固化
      return Provider.GEMINI;
