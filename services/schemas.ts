...
export function safeParseAiJson<T>(
  ...
  try {
    ...
    console.error(
      `[Zod] ${label}: JSON.parse failed. Raw text (first 500 chars):`,
  ...
  if (rawObj && typeof rawObj === 'object' && !Array.isArray(rawObj)) {
    ...
    if (obj.characters && Array.isArray(obj.characters)) {
      console.log(
        `[Zod] ${label}: Detected nested structure with 'characters' array, extracting...`
      );
    ...
    // 世界观生成: 提取 settings 或 worldSettings 数组
      ...
      console.log(
        `[Zod] ${label}: Detected nested structure with 'worldSettings' array, extracting...`
      );
    ...
    // 剧情生成: 提取 plotNodes 或 nodes 数组
      ...
      console.log(
        `[Zod] ${label}: Detected nested structure with 'plotNodes' array, extracting...`
      );
  ...
  if (result.success) {
    ...
    console.warn(
      `[Zod] ${label}: Validation failed. Issues:`,
      result.error.issues.map((i) => `${i.path.join('.')}: ${i.message}`)
    );
    console.warn(
      `[Zod] ${label}: Raw object (keys):`,
...
export const AiCharacterRelationSchema = z.object({
  ...
  type: z
    ...
    .optional()
    .describe('关系类型（可选，默认为 RELATED_TO）'),
...
export const AiCharacterSchema = z.object({
  ...
  structuredRelations: z
    .array(AiCharacterRelationSchema)
    .optional()
    .describe('结构化关系数组（新格式）'),
