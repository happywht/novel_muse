...
export function generateConflictVisualization(
  ...
  // 1. 基础数据准备
  const conflictNodes = plotNodes.filter((node) => node.beatTag?.includes('conflict'));

  // 2. 生成热力图数据
  const heatmap = generateHeatmapData(conflictNodes, chapters);

  // 3. 生成角色压力数据
  const characterStress = generateCharacterStressData(conflictNodes, chapters, characters);
 ...
 * 生成热力图数据
 */
function generateHeatmapData(conflictNodes: PlotNode[], chapters: Chapter[]): ConflictHeatmapData {
  ...
  chapters.forEach((chapter) => {
    ...
    // 主要冲突类型
    const primaryType =
      nodeConflicts.length > 0 ? determinePrimaryConflictType(nodeConflicts) : 'inner';
...
function generateCharacterStressData(
  ...
  return characters.map((character) => {
    ...
    chapters.forEach((chapter) => {
      ...
      // 计算角色在本章的压力
      const stress = calculateCharacterStress(character, chapterConflicts, chapter);
 ...
 * 生成冲突分布
 */
function generateConflictDistribution(conflictNodes: PlotNode[]): ConflictDistribution {
 ...
 * 生成时间线
 */
function generateTimeline(conflictNodes: PlotNode[], chapters: Chapter[]): ConflictTimeline {
 ...
 * 识别修罗场节点
 */
function identifyClimaxNodes(conflictNodes: PlotNode[], chapters: Chapter[]) {
...
function generateInsights(
  ...
  // 找出冲突最激烈的章节
  const maxIntensityIndex = heatmap.intensities.indexOf(Math.max(...heatmap.intensities));
  ...
  const densityStdDev = Math.sqrt(
    heatmap.conflictCounts.reduce(
      (sum, count) =>
        sum +
        Math.pow(
          count - heatmap.conflictCounts.reduce((a, b) => a + b, 0) / heatmap.conflictCounts.length,
          2
        ),
      0
    ) / heatmap.conflictCounts.length
  );
  ...
  const maxStressChar = characterStress.find((char) => char.peakChapters.length >= 3);
  if (maxStressChar) {
...
function extractConflictIntensity(node: PlotNode): number {
  ...
  // 默认强度
  return node.beatTag?.includes('climax')
    ? 9
    : node.beatTag?.includes('major')
      ? 7
      : node.beatTag?.includes('minor')
        ? 4
        : 5;
}
 ...
 * 判断角色是否参与冲突
 */
function isCharacterInvolvedInConflict(character: Character, conflict: PlotNode): boolean {
  // 检查冲突相关角色
