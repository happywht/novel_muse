...
export const getGlobalConfig = async (): Promise<typeof DEFAULT_CONFIG> => {
  ...
  configLoadPromise = (async () => {
    try {
      const savedConfig =
        await storageService.getItem<typeof DEFAULT_CONFIG>(STORAGE_KEY_GLOBAL_CONFIG);
      if (savedConfig) {
        // 合并保存的配置和默认配置
...
export const getAIClient = async () => {
  ...
  const apiKey =
    config.ai.providers.gemini.apiKey || process.env.API_KEY || process.env.GEMINI_API_KEY;
...
export const getModelName = async (tier: 'flash' | 'pro' = 'flash'): Promise<string> => {
  ...
  if (tier === 'pro') {
    return (
      config.ai.providers.gemini.proModel || process.env.GEMINI_PRO_MODEL || 'gemini-3-pro-preview'
    );
  } else {
    return (
      config.ai.providers.gemini.flashModel ||
...
export async function retryOperation<T>(
  ...
  try {
    ...
    const errorCode =
      (error as any)?.status || (error as any)?.code || (error as any)?.response?.status;
    ...
    if (retries > 0 && isRetryable) {
      console.warn(
        `API call failed with code ${errorCode}. Retrying in ${delay}ms... (Retries left: ${retries})`
      );
