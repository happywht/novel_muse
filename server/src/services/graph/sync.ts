...
function sanitizeRelationType(relation: string): string {
  ...
  // 5. 不在白名单中，使用默认类型
  console.warn(
    `[Graph Security] Unknown relation type "${relation}" -> normalized to "${normalized}" -> fallback to RELATED_TO`
  );
  return 'RELATED_TO';
}
...
const doSyncProject = async (projectData: any): Promise<void> => {
  ...
  try {
    ...
    await session.run(`MATCH (n {projectId: $projectId}) DETACH DELETE n`, { projectId });
    ...
    if (projectData.characters?.length > 0) {
      ...
      try {
        ...
        for (const char of projectData.characters) {
          if (
            char.structuredRelations &&
            Array.isArray(char.structuredRelations) &&
            char.structuredRelations.length > 0
          ) {
        ...
        if (!hasStructuredData) {
          console.log(
            `🔄 No structuredRelations found, falling back to AI extraction for project ${projectId}`
          );
          const extractedRelations = await graphLlm.extractCharacterRelationships(
        ...
        if (triplesToSync.length > 0) {
          for (const triple of triplesToSync) {
            try {
              ...
              console.warn(
                `Failed to sync relation ${triple.subject}-[:${triple.relation}]->${triple.object}:`,
                err
              );
            }
          }
          console.log(
            `✅ Synced ${triplesToSync.length} character relationships for project ${projectId} (source: ${hasStructuredData ? 'STRUCTURED_DATA' : 'AI_EXTRACTED'})`
          );
        }
      } catch (syncErr) {
      ...
      try {
        ...
        for (const node of projectData.plotNodes) {
          await session.run(
                             ...
                             pn.relatedChapters = $relatedChapters`,
            {
              ...
              conflictScenario: node.conflictScenario
                ? JSON.stringify(node.conflictScenario)
                : null,
              // 新增: 关联章节 (JSON数组)
          ...
          if (node.relatedCharacters?.length > 0) {
            for (const charId of node.relatedCharacters) {
              try {
                ...
                console.warn(
                  `Failed to link PlotNode ${node.id} to Character ${charId}:`,
                  charRelErr
                );
              }
            }
          ...
          if (node.relatedLocations?.length > 0) {
            for (const locId of node.relatedLocations) {
              try {
                ...
                console.warn(
                  `Failed to link PlotNode ${node.id} to WorldSetting ${locId}:`,
                  locRelErr
                );
              }
            }
        ...
        for (let i = 0; i < sortedNodes.length - 1; i++) {
          try {
            ...
            console.warn(
              `Failed to create PRECEDES relationship for PlotNodes ${sortedNodes[i].id} -> ${sortedNodes[i + 1].id}:`,
              precErr
            );
          }
        }
        ...
        for (const node of projectData.plotNodes) {
          ...
          if (node.conflictScenario !== null && node.conflictScenario !== undefined) {
            if (node.conflictScenario.participants?.length > 0) {
              ...
              console.log(
                `  └─ Conflict scenario: ${conflict.participants.length} participants, intensity ${conflict.intensity}, type ${conflict.type}`
              );
            }
          }
        }
    ...
    if (projectData.worldSettings?.length > 0) {
      for (const ws of projectData.worldSettings) {
        ...
        const categoryLabel =
          (ws.category?.replace(/[^a-zA-Z0-9_]/g, '_') || 'Other').replace(/^_+|_+$/g, '') ||
          'Other';
        await session.run(
    ...
    if (projectData.timeline?.length > 0) {
      for (const evt of projectData.timeline) {
        ...
        const entities: string[] = Array.isArray(evt.involvedEntities) ? evt.involvedEntities : [];
    ...
    if (projectData.echoes?.length > 0) {
      const acceptedEchoes = projectData.echoes.filter((e: any) => e.status === 'ACCEPTED');
        ...
        if (echo.triples && Array.isArray(echo.triples)) {
          for (const triple of echo.triples) {
            ...
            try {
              ...
              console.warn(
                `Failed to sync triple ${triple.subject}-[:${relType}]->${triple.object}:`,
                err
              );
            }
          }
    ...
    console.log(
      `📊 Graph synced for project ${projectId}: ${projectData.characters?.length || 0} chars, ${projectData.worldSettings?.length || 0} settings, ${projectData.chapters?.length || 0} chapters, ${projectData.plotNodes?.length || 0} plot nodes`
    );
  } finally {
    await session.close();
  }
 ...
 */
export const syncEchoToGraph = async (echo: {
  id: string;
  projectId: string;
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[syncEchoToGraph] Created AFFECTS relationship: Echo ${id} -> ${targetLabel} ${targetId}`
      );
    } catch (err) {
      ...
      for (const triple of triples) {
        try {
          ...
          if (triple.isForeshadowing) {
            console.log(
              `[syncEchoToGraph] Foreshadowing detected: ${triple.subject} -[${relType}]-> ${triple.object}`
            );
          }
        } catch (err) {
          failCount++;
 ...
 */
export const syncChapterToGraph = async (chapter: {
  id: string;
  projectId: string;
  title: string;
  content?: string;
  ...
  try {
    ...
    if (plotNodeId) {
      try {
        ...
        console.log(
          `[syncChapterToGraph] Created EXPANDS_TO relationship: PlotNode ${plotNodeId} -> Chapter ${id}`
        );
      } catch (err) {
        console.warn(
          `[syncChapterToGraph] Failed to create EXPANDS_TO relationship for Chapter ${id}:`,
          err
        );
        // 继续执行，即使关系创建失败
      }
    ...
    if (expectedPOV) {
      try {
        ...
        console.log(
          `[syncChapterToGraph] Created POV_IS relationship: Chapter ${id} -> Character ${expectedPOV}`
        );
      } catch (err) {
        console.warn(
          `[syncChapterToGraph] Failed to create POV_IS relationship for Chapter ${id}:`,
          err
        );
        // 继续执行，即使关系创建失败
      }
    ...
    if (beats && Array.isArray(beats) && beats.length > 0) {
      ...
      for (let i = 0; i < beats.length; i++) {
        ...
        try {
          ...
          failCount++;
          console.warn(
            `[syncChapterToGraph] Failed to sync Beat ${beat.id} for Chapter ${id}:`,
            err
          );
        }
      }
      ...
      console.log(
        `[syncChapterToGraph] Beat sync complete for Chapter ${id}: ${successCount} succeeded, ${failCount} failed`
      );
    }
...
export const syncChaptersToGraph = async (
  ...
  console.log(
    `[syncChaptersToGraph] Batch sync complete: ${successCount} succeeded, ${failCount} failed`
  );
};
 ...
 * @returns 同步统计信息
 */
export const syncForgeResult = async (result: {
  projectId: string;
  chapterId: string;
  echoes: Array<{
    id: string;
  ...
  console.log(
    `[syncForgeResult] Processing ${echoes?.length || 0} echoes, ${physicalStatusUpdates?.length || 0} status updates`
  );
    ...
    for (const echo of echoes) {
      try {
        ...
        try {
          ...
          console.log(
            `[syncForgeResult] Created GENERATES relationship: Chapter ${chapterId} -> Echo ${echo.id}`
          );
        } catch (err) {
  ...
  if (
    ...
    Array.isArray(physicalStatusUpdates) &&
    physicalStatusUpdates.length > 0
  ) {
    const d = getDriver();
    const session = d.session();
    ...
    try {
      for (const update of physicalStatusUpdates) {
        try {
          ...
          updatedCharacters++;
          console.log(
            `[syncForgeResult] Updated physical status for Character ${update.characterId}`
          );
          ...
          try {
            ...
            console.log(
              `[syncForgeResult] Created INVOLVES relationship: Chapter ${chapterId} -> Character ${update.characterId}`
            );
          } catch (err) {
      ...
      for (const echo of echoes) {
        try {
          if (echo.type === 'CHARACTER') {
            ...
            console.log(
              `[syncForgeResult] Created INVOLVES relationship: Chapter ${chapterId} -> Character ${echo.targetId}`
            );
            ...
            console.log(
              `[syncForgeResult] Created REFERENCES relationship: Chapter ${chapterId} -> WorldSetting ${echo.targetId}`
            );
          }
        } catch (err) {
  ...
  console.log(
    `[syncForgeResult] Sync complete: ${syncedEchoes} echoes, ${syncedTriples} triples, ${updatedCharacters} character updates`
  );
  if (errors.length > 0) {
