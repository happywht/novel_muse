      ...
      if (r && m) {
        ...
        if (!seenRelationIds.has(rId)) {
          ...
          if (!seenEntityIds.has(m.properties.id)) {
            ...
            entitiesStr.push(
              `[${mName}]: ${mDesc.slice(0, 150)}${mDesc.length > 150 ? '...' : ''}`
            );
 ...
 */
export const inferNarrativeInsights = async (projectId: string): Promise<NarrativeInsight[]> => {
  const d = getDriver();
  const session = d.session();
 ...
 */
export const getUnresolvedForeshadowing = async (
  projectId: string,
  branchId: string = 'main'
...
export const simulateStatePropagation = async (
  ...
  try {
    ...
    for (const record of result.records) {
      ...
      const weight =
        typeof rawWeight === 'number' ? rawWeight : rawWeight.toNumber ? rawWeight.toNumber() : 50;
      ...
      if (['ALLY_OF', 'LOVES', 'KIN_OF', 'MENTORS'].includes(relType)) {
        ...
        reason = isNegative
          ? `由于盟友 ${triggerName} 受损，${target} 的后援被削弱`
          : `${triggerName} 的好转增强了其盟友 ${target} 的地位`;
      } else if (['ENEMY_OF'].includes(relType)) {
        impact = isNegative ? 'POSITIVE' : 'NEGATIVE';
        reason = isNegative
          ? `宿敌 ${triggerName} 的衰落给 ${target} 留下了可乘之机`
          : `敌对势力 ${triggerName} 的增强对 ${target} 构成了更大威胁`;
      }
