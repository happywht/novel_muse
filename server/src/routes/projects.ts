...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const state = {
      ...
      availableBranches: (project as any).availableBranches
        ? JSON.parse((project as any).availableBranches)
        : undefined,
...
router.put('/:id/full', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx: any) => {
      ...
      await tx.project.upsert({
        ...
        create: {
          ...
          styleTags: data.creativeSettings?.styleTags
            ? JSON.stringify(data.creativeSettings.styleTags)
            : null,
      ...
      // 删除不再存在的角色
      const characterIdsToDelete = Array.from(existingCharacterIds).filter(
        (cid) => !incomingCharacterIds.has(cid)
      );
      ...
      for (const char of data.characters || []) {
        const charData = {
          ...
          controlledTerritories: char.controlledTerritories
            ? JSON.stringify(char.controlledTerritories)
            : null,
          ...
          foreshadowingHooks: char.foreshadowingHooks
            ? JSON.stringify(char.foreshadowingHooks)
            : null,
      ...
      const existingWorldSettingIds = new Set(
        existingWorldSettings.map((w: { id: string }) => w.id)
      );
      const incomingWorldSettingIds = new Set((data.worldSettings || []).map((w: any) => w.id));

      const worldSettingIdsToDelete = Array.from(existingWorldSettingIds).filter(
        (wid) => !incomingWorldSettingIds.has(wid)
      );
      ...
      const plotVersionIdsToDelete = Array.from(existingPlotVersionIds).filter(
        (pid) => !incomingPlotVersionIds.has(pid)
      );
      ...
      const draftIdsToDelete = Array.from(existingDraftIds).filter(
        (did) => !incomingDraftIds.has(did)
      );
      ...
      const chapterIdsToDelete = Array.from(existingChapterMap.keys()).filter(
        (chid) => !incomingChapterIds.has(chid)
      );
      ...
      for (const ch of data.chapters || []) {
        ...
        const contentToSave =
          ch.content === '' && existingChapter && existingChapter.content !== ''
      ...
      const plotNodeIdsToDelete = Array.from(existingPlotNodeIds).filter(
        (pnid) => !incomingPlotNodeIds.has(pnid)
      );
      ...
      const echoIdsToDelete = Array.from(existingEchoIds).filter(
        (eid) => !incomingEchoIds.has(eid)
      );
      ...
      const existingTimelineEventIds = new Set(
        existingTimelineEvents.map((t: { id: string }) => t.id)
      );
      const incomingTimelineEventIds = new Set((data.timeline || []).map((t: any) => t.id));

      const timelineEventIdsToDelete = Array.from(existingTimelineEventIds).filter(
        (tid) => !incomingTimelineEventIds.has(tid)
      );
    ...
    syncProjectToGraph(data).catch((err) => console.warn('Graph sync skipped:', err.message));
...
router.patch('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (data.creativeSettings) {
      ...
      if (data.creativeSettings.creativity !== undefined)
        updateData.creativity = Number(data.creativeSettings.creativity);
      if (data.creativeSettings.targetAudience !== undefined)
        updateData.targetAudience = data.creativeSettings.targetAudience;
      if (data.creativeSettings.promptProfile !== undefined)
        updateData.promptProfile = data.creativeSettings.promptProfile;
      if (data.creativeSettings.styleTags !== undefined)
        updateData.styleTags = data.creativeSettings.styleTags
          ? JSON.stringify(data.creativeSettings.styleTags)
          : null;
      if (data.creativeSettings.referenceText !== undefined)
        updateData.referenceText = data.creativeSettings.referenceText || null;
    }

    // World Gen Config (flattened in DB)
    if (data.worldGenConfig) {
      if (data.worldGenConfig.detailLevel !== undefined)
        updateData.detailLevel = data.worldGenConfig.detailLevel;
    ...
    if (data.availableBranches !== undefined)
      updateData.availableBranches = data.availableBranches
        ? JSON.stringify(data.availableBranches)
        : null;
...
router.get('/:id/statistics', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const totalWords = chapters.reduce(
      (sum: number, ch: any) => sum + (ch.content?.length || 0),
      0
    );
