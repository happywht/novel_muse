...
router.get('/:projectId/subgraph', async (req: Request, res: Response) => {
  ...
  try {
    const subgraph = await getRelatedSubgraph(
...
router.get('/:projectId/physical-status', async (req: Request, res: Response) => {
  ...
  try {
    const status = await getPhysicalStatus(
      req.params.projectId as string,
      anchorList,
      branchId as string
    );
...
router.get('/:projectId/foreshadowing', async (req, res) => {
  try {
    ...
    const foreshadowing = await getUnresolvedForeshadowing(
      req.params.projectId as string,
      branchId
    );
...
router.post('/:projectId/propagate', async (req: Request, res: Response) => {
  try {
    ...
    const risks = await simulateStatePropagation(
      req.params.projectId as string,
      triggerName,
      changeDescription || ''
    );
...
router.get(
  '/:projectId/characters/:characterId/foreshadowing',
...
router.get(
  '/:projectId/characters/:characterId/physical-status',
  async (req: Request, res: Response) => {
    try {
...
router.get('/:projectId/echoes/foreshadowing', async (req: Request, res: Response) => {
  try {
    ...
    const foreshadowing = await getEchoForeshadowing(req.params.projectId as string, branchId);
    res.json(foreshadowing);
  } catch (err: any) {
...
router.get(
  '/:projectId/chapters/:chapterId/character-network',
  async (req: Request, res: Response) => {
    try {
...
router.get(
  '/:projectId/chapters/:chapterId/foreshadowing-chain',
  async (req: Request, res: Response) => {
...
router.get(
  '/:projectId/characters/:characterId/location-context',
  async (req: Request, res: Response) => {
    try {
...
router.get(
  '/:projectId/world-settings/:locationId/characters',
  async (req: Request, res: Response) => {
    try {
...
router.get('/:projectId/forge-context', async (req: Request, res: Response) => {
  try {
    ...
    const forgeContext = await getForgeContext(req.params.projectId as string, {
