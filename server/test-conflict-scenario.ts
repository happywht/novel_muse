...
import { syncProjectToGraph, getCharacterConflicts, getHighIntensityConflicts } from './src/services/graph';
...
async function testConflictScenario() {
    ...
    try {
        ...
        char1Conflicts.forEach((conflict, idx) => {
            ...
            console.log(`      - 其他参与者: ${conflict.otherParticipants.map((p: any) => p.name).join(', ')}`);
        });
        console.log('   ✓ 查询成功\n');
    } catch (err) {
