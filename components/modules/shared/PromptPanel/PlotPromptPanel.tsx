...
export const PlotPromptPanel: React.FC<PlotPromptPanelProps> = ({ isModal, onClose }) => {
    ...
    // 处理保存
    const handleSave = useCallback((key: string, content: string, level: 'PROJECT' | 'MODULE') => {
    ...
    // 处理重置
    const handleReset = useCallback((key: string, level: 'PROJECT' | 'MODULE') => {
    ...
    // 处理全部重置
    const handleResetAll = useCallback((level: 'PROJECT' | 'MODULE') => {
