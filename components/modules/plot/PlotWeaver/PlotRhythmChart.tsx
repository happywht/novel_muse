...
export const PlotRhythmChart: React.FC<PlotRhythmChartProps> = ({
    ...
    const taggedNodes = project.plotNodes
        .filter(n => n.beatTag)
                    ...
                    {taggedNodes.map((node, idx) => {
                        ...
                        return (
                            <div key={node.id} className="relative group">
                                {/* Timeline Dot */}
                                <div className={`absolute -left-[41px] top-1 w-4 h-4 rounded-full border-2 border-slate-900 shadow-xl transition-transform group-hover:scale-125 z-10 ${tag ? tag.activeColor : 'bg-slate-700'}`} />
                                ...
                                <div className="bg-slate-900/80 border border-slate-800/50 rounded-xl p-4 transition-all hover:bg-slate-800/80 hover:border-muse-500/30 group-hover:translate-x-1 shadow-lg">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${tag?.color}`}>
                                            {tag?.label}
                                        </span>
                                        <ChevronRight size={10} className="text-slate-600" />
                                        <h4 className="text-xs font-bold text-slate-200 truncate">{node.title || "未命名情节"}</h4>
                                    </div>
