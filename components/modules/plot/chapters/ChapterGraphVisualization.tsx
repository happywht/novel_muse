...
export const ChapterGraphVisualization: React.FC<ChapterGraphVisualizationProps> = ({
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setError(`加载${mode === 'dependencies' ? '依赖关系' : mode === 'network' ? '角色网络' : '冲突热力图'}失败`);
        } finally {
            setLoading(false);
        }
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-slate-800/30 rounded-xl p-4 border border-slate-700/30">
                                        <div className="flex items-center gap-2 text-violet-400 mb-3">
                                            <Users className="w-4 h-4" />
                                            <span className="font-semibold">
                                                角色列表 ({network.characters.length})
                                            </span>
                                        </div>
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className={`${getRelationColor(rel.relation)} font-medium`}>
                                                                {RELATION_TYPE_LABELS[rel.relation as CharacterRelationType] || rel.relation}
                                                            </span>
                                                            <ArrowRight className="w-4 h-4 text-slate-600" />
                                            ...
                                            {heatmap.map(item => (
                                                ...
                                                >
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="text-xs text-slate-500">
                                                            {item.conflictType || '无冲突'} | 参与者: {item.participants?.join(', ') || '无'}
                                                        </div>
                                                    </div>
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${getIntensityColor(item.intensity)}`}>
                                                        {item.intensity}
                                                    </div>
                                                </div>
                                            ))}
