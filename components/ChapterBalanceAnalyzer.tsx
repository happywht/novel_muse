...
import {
  analyzeChapterBalance,
  generateOptimizationSuggestions,
  BalanceReport,
  OptimizationSuggestion,
} from '../services/chapterBalanceAlgorithm';
import {
  Wand2,
  AlertCircle,
  CheckCircle,
...
export const ChapterBalanceAnalyzer: React.FC<ChapterBalanceAnalyzerProps> = ({
  ...
  const [suggestions] = useState(() =>
    generateOptimizationSuggestions(report, chapters, characters)
  );
  const [selectedTab, setSelectedTab] = useState<
    'overview' | 'wordcount' | 'conflict' | 'characters' | 'pov' | 'suggestions'
  >('overview');
  ...
  const getSuggestionIcon = (type: OptimizationSuggestion['type']) => {
    switch (type) {
      ...
      case 'REDUCE_CONFLICT':
        return <Zap className="w-4 h-4 opacity-50" />;
    ...
    <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-center">
          <div
            className={`text-4xl font-bold ${getOverallScoreColor(report.overallBalance.score)}`}
          >
                ...
                <div className="bg-orange-900/20 border border-orange-800/50 rounded-lg p-4">
                  ...
                  <ul className="space-y-1 text-sm text-slate-300">
                    {report.conflictAnalysis.hotspots.map((id) => {
                      ...
                      return chapter ? <li key={id}>{chapter.title}</li> : null;
                    })}
                  </ul>
                </div>
              )}
                  ...
                  .map((char) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div
                          className={`font-bold ${
                            char.balanceScore >= 80
                              ? 'text-emerald-400'
                              : char.balanceScore >= 60
                                ? 'text-yellow-400'
                                : 'text-red-400'
                          }`}
                        >
                ...
                <div className="space-y-3">
                  {Object.entries(report.povDistribution.chaptersByPOV).map(([pov, count]) => (
                    ...
                    >
                      ...
                      <span className="text-lg font-bold text-muse-400">{count} 章</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col justify-center items-center">
                  <div
                    className={`text-2xl font-bold ${
                      report.povDistribution.rotationPattern === 'GOOD'
                        ? 'text-emerald-400'
                        : report.povDistribution.rotationPattern === 'FAIR'
                          ? 'text-yellow-400'
                          : 'text-red-400'
                    }`}
                  >
              ...
              {suggestions.map((suggestion, idx) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`p-2 rounded-lg border ${getPriorityColor(suggestion.priority)}`}
                    >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full border ${getPriorityColor(suggestion.priority)}`}
                        >
                          {suggestion.priority}
                        </span>
                      </div>
