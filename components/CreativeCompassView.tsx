...
export const CreativeCompassView: React.FC<CreativeCompassViewProps> = ({
  project,
  updateProject,
}) => {
    ...
    <div className="h-full flex flex-col bg-[#0f172a] animate-fade-in relative z-10 p-6 overflow-y-auto custom-scrollbar">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm max-w-2xl">
          宏观把控作品基调，微观调校 AI
          笔触与描写倾向。在此设定的罗盘指向将深刻影响整个宇宙的情节推演与文本生成。
        </p>
      </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span className="text-[10px] text-center opacity-70 leading-relaxed">
                    节奏极快 / 强爽感
                    <br />
                    简单直接
                  </span>
                  ...
                  {PREDEFINED_STYLES.map((tag) => (
                    <button
                      ...
                      className={`text-xs px-2.5 py-1 rounded-md border transition-all ${
                        (project.creativeSettings.styleTags || []).includes(tag)
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Object.values(GENRE_RULES).map((rule) => (
                      <button
                        key={rule.id}
                        onClick={() =>
                          updateProject({ genre: rule.id === project.genre ? '' : rule.id })
                        }
                  ...
                  {project.genre && GENRE_RULES[project.genre] && (
                    <div className="mt-2 text-[9px] text-cyan-400/60 bg-cyan-900/15 p-2 rounded border border-cyan-500/15">
                      已激活：疲劳词{GENRE_RULES[project.genre].fatigueWords.length}个、禁忌
                      {GENRE_RULES[project.genre].taboos.length}条、语言铁律
                      {GENRE_RULES[project.genre].languageRules.length}条
                    </div>
                  )}
                </div>
              </div>
            ...
            {styleProfile && (
              <div className="mt-4 bg-emerald-900/15 border border-emerald-500/25 rounded-xl p-4 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                  ...
                  <div className="bg-slate-900/50 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-emerald-300">
                      {styleProfile.sentenceLengthStdDev}
                    </div>
                    <div className="text-[9px] text-slate-500">句长标准差</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-emerald-300">
                      {styleProfile.avgParagraphLength}
                    </div>
                    <div className="text-[9px] text-slate-500">平均段长</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-emerald-300">
                      {(styleProfile.vocabularyDiversity * 100).toFixed(1)}%
                    </div>
                ...
                {styleProfile.topPatterns.length > 0 && (
                  <div className="text-[10px] text-slate-400 mb-2">
                    ...
                    {styleProfile.topPatterns.map((p, i) => (
                      <span
                        key={i}
                        className="ml-2 text-emerald-300/70 bg-emerald-900/20 px-1.5 py-0.5 rounded"
                      >
                        {p}
                      </span>
                    ))}
                ...
                {styleProfile.rhetoricalFeatures.length > 0 && (
                  <div className="text-[10px] text-slate-400">
                    ...
                    {styleProfile.rhetoricalFeatures.map((f, i) => (
                      <span
                        key={i}
                        className="ml-2 text-amber-300/70 bg-amber-900/20 px-1.5 py-0.5 rounded"
                      >
                        {f}
                      </span>
                    ))}
