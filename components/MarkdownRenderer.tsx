...
export const MarkdownRenderer: React.FC<{ content: string; className?: string }> = ({
  ...
  if (
    ...
    content.trim().startsWith('<h') ||
    content.trim().startsWith('<ul>') ||
    content.trim().startsWith('<li>')
  ) {
  ...
  const sections = content.split('\n\n').map((block, index) => {
    ...
    if (block.startsWith('## ')) {
      return (
        <h3 key={index} className="text-xl font-bold text-muse-300 mb-2 mt-4">
          {block.replace('## ', '')}
        </h3>
      );
    }
    if (block.startsWith('# ')) {
      return (
        <h2 key={index} className="text-2xl font-bold text-muse-400 mb-3 mt-5">
          {block.replace('# ', '')}
        </h2>
      );
    }
    if (block.startsWith('### ')) {
      return (
        <h4 key={index} className="text-lg font-semibold text-muse-200 mb-2 mt-3">
          {block.replace('### ', '')}
        </h4>
      );
    }
      ...
      const items = block.split('\n').map((line, i) => {
        ...
        const parts = cleanLine.split(/(\*\*.*?\*\*)/g).map((part, j) => {
          if (part.startsWith('**') && part.endsWith('**')) {
            return (
              <strong key={j} className="text-muse-100">
                {part.slice(2, -2)}
              </strong>
            );
          }
    ...
    const parts = block.split(/(\*\*.*?\*\*)/g).map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return (
          <strong key={i} className="text-muse-100">
            {part.slice(2, -2)}
          </strong>
        );
      }
    ...
    return (
      <p key={index} className="mb-4 text-slate-300 leading-relaxed font-serif">
        {parts}
      </p>
    );
  });
