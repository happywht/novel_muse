...
export const EchoEventFeed: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'ARCHIVED':
        return <History size={14} className="text-slate-500" />;
      default:
      ...
      <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/30">
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-slate-800 rounded-lg p-1 ml-4 shadow-inner">
            ...
            >
              <Inbox size={14} /> 收件箱{' '}
              {project.echoes.filter((e) => e.status === 'PENDING').length > 0 && (
        ...
        >
          {isDeducing ? (
            ...
            <>
              <Sparkles size={16} /> <span className="text-xs font-bold">推演蝴蝶效应</span>
            </>
          )}
        </button>
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg ${echo.type === 'CHARACTER' ? 'bg-indigo-900/30 text-indigo-400' : 'bg-emerald-900/30 text-emerald-400'}`}
                  >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-500 uppercase tracking-wider font-medium">
                      {getStatusIcon(echo.status)} 实录回响 -{' '}
                ...
                {echo.triples && echo.triples.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {echo.triples.map((t, i) => (
                      <div
                        key={i}
                        className="flex flex-col gap-1 bg-slate-900/80 border border-muse-500/20 p-2 rounded text-[10px]"
                      >
