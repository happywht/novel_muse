...
import {
    ...
    FileCode, Cloud, Database // NEW: template editor icon + data source icons
} from 'lucide-react';
...
interface SidebarProps {
    ...
    useBackend?: boolean; // 新增：数据存储模式
    isOnline?: boolean; // 新增：网络状态
}
    ...
    return (
        ...
        >
            ...
            <div className="h-14 flex items-center px-4 border-b border-slate-800/80 gap-3 shrink-0">
                ...
                {/* 数据来源指示器 */}
                <div className={`ml-auto transition-all duration-300 ${expanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
                    <div className="flex items-center gap-1.5 text-[10px] px-2 py-1 rounded-lg border bg-opacity-10">
                        {useBackend ? (
                            <>
                                <Cloud className="w-3 h-3 text-emerald-400" />
                                <span className="text-emerald-400 font-medium">云端</span>
                            </>
                        ) : (
                            <>
                                <Database className="w-3 h-3 text-amber-400" />
                                <span className="text-amber-400 font-medium">本地</span>
                            </>
                        )}
                        {!isOnline && (
            ...
            <div className="py-2 px-2 border-b border-slate-800/80">
                <button
                    ...
                    onKeyDown={(e) => handleKeyDown(e, () => setActiveSection(AppSection.LOBBY))}
                    tabIndex={activeSection === AppSection.LOBBY ? 0 : -1}
                    aria-label="返回项目大厅"
                    aria-current={activeSection === AppSection.LOBBY ? 'page' : undefined}
            ...
            {/* Navigation Items */}
            <nav
                className="flex-1 py-3 px-2 space-y-1 overflow-y-auto custom-scrollbar"
                role="navigation"
                aria-label="主要功能导航"
            >
                {NAV_ITEMS.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            onKeyDown={(e) => handleKeyDown(e, () => setActiveSection(item.id))}
            ...
            {/* Bottom Tools */}
            <div
                className="py-3 px-2 space-y-1 shrink-0"
                role="navigation"
                aria-label="工具和设置"
            >
                {tools.map((tool) => {
                    ...
                    return (
                        <div key={tool.label} className="relative group/tool">
                            <button
                                ...
                                onKeyDown={(e) => handleKeyDown(e, tool.onClick)}
