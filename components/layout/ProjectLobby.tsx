...
import { FolderOpen, Plus, Upload, Download, Trash2, Clock, BookOpen, Users, Calendar } from 'lucide-react';
            ...
            <main className="flex-1 px-8 max-w-7xl mx-auto w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sortedProjects.map(project => (
                        <div
                            key={project.id}
                            className={`group relative bg-slate-900/40 border rounded-3xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-muse-500/5 hover:-translate-y-1 ${currentProjectId === project.id
                        ...
                        >
                            ...
                            >
                                <div className="mb-6 flex items-start justify-between">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${currentProjectId === project.id ? 'bg-muse-500/20 text-muse-400' : 'bg-slate-800 text-slate-500'
                                        }`}>
                                        <BookOpen size={24} />
                                    </div>
                    ...
                    >
                        ...
                        <span className="font-bold text-slate-500 group-hover:text-muse-400 transition-colors">拓展新纪元</span>
                    </button>
                </div>
            </main>
        </div>
