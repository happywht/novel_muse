...
export const WritingStats: React.FC<WritingStatsProps> = ({ project }) => {
  const stats = useMemo(() => {
    ...
    const progress = nextMilestone
      ? ((totalWords - (currentMilestone?.threshold || 0)) /
          (nextMilestone.threshold - (currentMilestone?.threshold || 0))) *
        100
      : 100;
    ...
    <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-bold text-white text-lg">
                {stats.currentMilestone
                  ? `${stats.currentMilestone.icon} ${stats.currentMilestone.label}`
                  : '🌱 创作之旅开始'}
              </h3>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard
          ...
          sub={`共 ${stats.chapterWords.toLocaleString()} 字`}
        ...
        <StatCard
          ...
          sub={`共 ${stats.draftWords.toLocaleString()} 字`}
          color="text-blue-400"
        />
        <StatCard
          icon={Users}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard
          icon={TrendingUp}
          label="平均章节字数"
          value={stats.avgChapterWords}
          color="text-amber-400"
        />
        <StatCard
          icon={Flame}
          label="最长章节"
          value={stats.longestChapter.toLocaleString() + ' 字'}
          color="text-red-400"
        />
        <StatCard
          icon={Star}
          label="命运回响"
          value={stats.echoCount}
          sub="已捕获的状态变化"
        ...
        <StatCard
          icon={Clock}
          label="最近更新"
          value={stats.daysSinceUpdate === 0 ? '今天' : `${stats.daysSinceUpdate} 天前`}
            ...
            return (
              ...
              >
                ...
                <div className="text-[10px] text-slate-500 mt-1">
                  {(m.threshold / 1000).toLocaleString()}k 字
                </div>
