...
import { ProjectState, Character, WorldSetting, Draft, Chapter } from '../types';
import { generateSceneFromIngredients, analyzeStateChanges, StateChangeRecommendation, PacingMode, polishDraft, PolishMode } from '../services/geminiService';
...
export const DraftingRoom: React.FC<DraftingRoomProps> = ({ project, updateProject }) => {
  ...
  const handleGenerate = async () => {
      ...
      try {
          ...
          triggerStateAnalysis(result, activeCharacters, activeLocation);
  ...
  const triggerStateAnalysis = async (content: string, chars: Character[], location: WorldSetting | null) => {
      setIsAnalyzingState(true);
      try {
          const changes = await analyzeStateChanges(content, chars, location);
          if (changes.length > 0) {
       ...
       {viewMode === 'FORGE' ? (
           <>
               ...
               <div className="w-1/3 flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-2 pt-10">
                   ...
                   {/* Pacing Control (New) */}
                   <div className="bg-slate-800/50 p-2 rounded-xl border border-slate-700 flex justify-between items-center gap-1">
                       <button 
