...
export const AuxiliaryDrawer: React.FC<AuxiliaryDrawerProps> = ({
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-[450px] bg-slate-900 border-l border-slate-800 shadow-2xl z-40 transform transition-transform duration-500 ease-in-out flex flex-col ${showRightSidebar ? 'translate-x-0' : 'translate-x-full'}`}
    >
      {/* Header */}
      <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/80 backdrop-blur-md">
        <div className="flex items-center gap-3">
          {activeTab === 'ANALYSIS' && (
            <>
              <Activity className="text-muse-400" size={20} />{' '}
              <span className="font-bold text-white uppercase tracking-wider text-sm italic">
                剧情诊断报告
              </span>
            </>
          )}
        ...
        <button
          ...
          className="text-slate-500 hover:text-white p-1.5 hover:bg-slate-800 rounded-xl transition-all"
        >
          <X size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative p-6">
            ...
            <div className="mt-8 p-5 bg-slate-900/70 border border-red-800/30 rounded-2xl">
              ...
              {project.plotNodes.filter((n: PlotNode) => n.conflictScenario).length === 0 ? (
                ...
                <div className="space-y-4">
                  {project.plotNodes
                    .filter((n: PlotNode) => n.conflictScenario)
                    .map((conflictNode: PlotNode) => (
                      <div
                        key={conflictNode.id}
                        className="p-4 bg-slate-950/50 border border-red-800/20 rounded-xl"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-sm font-bold text-red-300 truncate">
                            {conflictNode.title}
                          </h4>
                          <span
                            className={`text-[10px] px-2 py-1 rounded-full font-bold ${
                              conflictNode.conflictScenario?.intensity >= 8
                                ? 'bg-red-500 text-red-950'
                                : conflictNode.conflictScenario?.intensity >= 6
                                  ? 'bg-orange-500 text-orange-950'
                                  : conflictNode.conflictScenario?.intensity >= 4
                          ...
                          <div className="flex flex-wrap gap-1">
                            {conflictNode.conflictScenario?.participants.map((charId) => {
                              ...
                              return char ? (
                                <span
                                  key={charId}
                                  className="text-[10px] px-2 py-1 rounded-full bg-red-900/30 border border-red-800 text-red-300"
                                >
                                  {char.name}
                                </span>
                              ) : null;
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-400">
                          ...
                          <span className="text-xs text-slate-300 capitalize">
                            {conflictNode.conflictScenario?.type === 'CONFRONTATION'
                              ? '正面对峙'
                              : conflictNode.conflictScenario?.type === 'CLIMAX'
                                ? '高潮冲突'
                                : '反转冲突'}
                          </span>
                        </div>
        ...
        {activeTab === 'PROMPT' && <PlotPromptPanel />}
