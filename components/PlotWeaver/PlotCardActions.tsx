      ...
      <div className="flex justify-between items-center">
        <div className="flex gap-2">
          <button
            onClick={() =>
              setShowEntitySelector(
                showEntitySelector?.id === node.id && showEntitySelector?.type === 'CHARACTER'
                  ? null
                  : { id: node.id, type: 'CHARACTER' }
              )
            }
          ...
          <button
            onClick={() =>
              setShowEntitySelector(
                showEntitySelector?.id === node.id && showEntitySelector?.type === 'LOCATION'
                  ? null
                  : { id: node.id, type: 'LOCATION' }
              )
            }
          ...
          <button
            onClick={() =>
              setShowConflictConfigurator(showConflictConfigurator === node.id ? null : node.id)
            }
            ...
            title={
              node.conflictScenario ? `修罗场: ${node.conflictScenario.stakes}` : '配置修罗场冲突'
            }
          >
        ...
        <div className="flex gap-2">
          <button
            ...
            title="AI 智能续写/扩写此段"
          >
            {isIterating && editingNodeId === node.id ? (
