...
const REWRITE_OPTIONS: RewriteOption[] = [
  {
    label: '🔥 增加冲突与张力',
    prompt: '增加剧情的冲突烈度，让反派更具压迫感，让主角的处境更绝望。',
  },
  {
    label: '🕵️‍♂️ 增加悬疑与反转',
    prompt: '埋下更多伏笔，并在结局或中点增加一个意想不到的剧情反转。',
  },
        ...
        {analysis ? (
          <div className="prose prose-invert prose-slate max-w-none animate-fade-in">
            ...
            <div className="mt-8 pt-4 border-t border-slate-700 flex flex-col items-center gap-4">
              ...
              >
                {isIterating ? <RefreshCw size={18} className="animate-spin" /> : <Zap size={18} />}
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-600 py-20">
            ...
            <p className="text-center max-w-xs text-sm leading-relaxed">
              点击侧边栏下方的<strong className="text-muse-400">“深度评估”</strong>按钮。
              <br />
              <br />
              AI 将穿透表象，从<strong className="text-muse-400">逻辑自洽</strong>、
              <strong className="text-muse-400">冲突强度</strong>与
              <strong className="text-muse-400">节奏张力</strong>
              维度进行全书审计，并给出可落地的重写建议。
            </p>
          </div>
        )}
      </>
    ...
    <div className="space-y-6 animate-fade-in">
      {selectedText && (
        <div className="bg-muse-900/20 border border-muse-500/30 p-3 rounded-lg mb-4">
          <div className="text-xs text-muse-400 font-bold mb-1 flex items-center gap-1">
            <MousePointer2 size={12} /> 正在针对选区操作
          </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-2">
          {REWRITE_OPTIONS.map((opt, idx) => (
            ...
            >
              <div className="font-medium text-slate-200 group-hover:text-muse-400">
                {opt.label}
              </div>
