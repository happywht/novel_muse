...
export const PlotCardHeader: React.FC = () => {
  const { node, idx, handleUpdateCard, handleRemoveCard, setFocusedNodeId } = usePlotCard();
        ...
        <div className="flex-1">
          ...
          <div className="relative mt-1">
            ...
            >
              ...
              <ChevronDown
                size={10}
                className={`transition-transform ${showBeatPicker ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {showBeatPicker && (
              <div className="absolute top-full left-0 mt-2 w-48 bg-slate-900 border border-slate-700 p-1.5 rounded-lg shadow-2xl z-20 animate-fade-in">
                <div className="grid grid-cols-1 gap-1">
                  <button
                    onClick={() => {
                      handleUpdateCard(node.id, { beatTag: null });
                  ...
                  {BEAT_TAGS.map((t) => (
                    <button
                      key={t.id}
                      onClick={() => {
                        handleUpdateCard(node.id, { beatTag: t.id as any });
                        setShowBeatPicker(false);
                      }}
