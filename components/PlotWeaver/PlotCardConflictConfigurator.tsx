...
export const PlotCardConflictConfigurator: React.FC = () => {
  ...
  const { node, project, showConflictConfigurator, setShowConflictConfigurator, handleUpdateCard } =
    usePlotCard();
    ...
    <div className="p-3 bg-slate-950 border border-red-800/50 rounded-lg animate-fade-in relative mt-3">
      ...
      <div className="mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <span
            className="text-[10px] text-slate-400 w-4 text-center"
            id={`intensity-value-${node.id}`}
          >
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          onClick={() => {
            const stakes =
              (document.getElementById(`stakes-${node.id}`) as HTMLInputElement)?.value || '';
            const intensity = parseInt(
              (document.getElementById(`intensity-${node.id}`) as HTMLInputElement)?.value || '7'
            );
            const type = (document.getElementById(`conflict-type-${node.id}`) as HTMLSelectElement)
              ?.value as 'CONFRONTATION' | 'CLIMAX' | 'TWIST';
        ...
        {node.relatedCharacters && node.relatedCharacters.length >= 2 && (
          <button
            onClick={async () => {
              ...
              try {
                const selectedChars = project.characters.filter((c) =>
                ...
                console.log(
                  '[修罗场] 匹配到的角色:',
                  selectedChars.map((c) => ({ id: c.id, name: c.name }))
                );
                console.log('[修罗场] 匹配到的角色数量:', selectedChars.length);

                if (selectedChars.length < 2) {
                  const missingIds =
                    node.relatedCharacters?.filter(
                      (id) => !project.characters.find((c) => c.id === id)
                    ) || [];
                  toast.error(
                    `生成失败：在项目中只找到 ${selectedChars.length} 个有效角色。缺失的角色ID: ${missingIds.join(', ')}。请检查相关角色是否已被删除或ID是否匹配。`,
                    8000
                  );
                  return;
                }
                ...
                if (incompleteChars.length > 0) {
                  toast.error(
                    `生成失败：以下角色信息不完整（缺少名称或描述）：${incompleteChars.map((c) => c.name || '未命名角色').join(', ')}`,
                    8000
                  );
                  return;
                }
