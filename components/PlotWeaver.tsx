...
export const PlotWeaver: React.FC<PlotWeaverProps> = ({ project, updateProject }) => {
  ...
  const updateProjectWithHistory = (data: Partial<ProjectState>, note: string) => {
    ...
    updateProject({
      ...data,
      plotHistory: [historyItem, ...(project.plotHistory || [])].slice(
        0,
        PLOT_CONFIG.MAX_HISTORY_ITEMS
      ),
    });
  };
  ...
  const sortedNodes = useMemo(
    () => [...project.plotNodes].sort((a, b) => a.order - b.order),
    [project.plotNodes]
  );
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] gap-6 relative">
                ...
                <VirtualList
                  ...
                  renderItem={(node, idx) => (
                    <PlotCard
                      ...
                      handleGenerateNodeAI={(id) =>
                        ai.handleGenerateNodeAI(id, setEditingNodeId, setDraftNodeContent)
                      }
                      handleQuickDraft={handleQuickDraft}
                      handleIterateNode={() =>
                        ai.handleIterateNode(
                          editingNodeId!,
                          iterationFeedback,
                          draftNodeContent!,
                          setDraftNodeContent
          ...
          {sortedNodes.length > 0 && (
            <div className="absolute bottom-4 right-4 pointer-events-none opacity-80">
              <div className="bg-slate-800/90 text-xs text-slate-400 px-3 py-1.5 rounded-full border border-slate-700 flex items-center gap-2 shadow-lg backdrop-blur-sm">
                ...
                <span>
                  AI 已连接: {project.characters.length} 角色 · {project.worldSettings.length} 设定
                </span>
              </div>
            </div>
          )}
      ...
      <AuxiliaryDrawer
        ...
        handleRewrite={(p, l) => {
          /* Optional: Add rewrite logic to hook if needed */
        }}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowSaveModal(false)} className="text-slate-400">
                取消
              </button>
              <button
                onClick={handleManualSave}
                className="bg-muse-600 px-4 py-2 rounded-lg text-white"
              >
                保存
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl">
            ...
            <p className="text-slate-300 text-sm mb-6">
              继续操作将覆盖现有内容（旧版本将自动存档）。是否继续？
            </p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setPendingAction(null)} className="text-slate-400">
                取消
              </button>
              <button
                onClick={confirmAction}
                className="bg-amber-600 px-4 py-2 rounded-lg text-white"
              >
                确认覆盖
              </button>
            </div>
      ...
      {toast && (
        <div
          className={`fixed bottom-8 left-1/2 -translate-x-1/2 px-6 py-3 rounded-full shadow-2xl z-50 transition-all animate-fade-in font-medium text-sm flex items-center gap-2 border ${toast.type === 'error' ? 'bg-red-500/10 border-red-500/50 text-red-200' : 'bg-emerald-500/10 border-emerald-500/50 text-emerald-200'}`}
        >
