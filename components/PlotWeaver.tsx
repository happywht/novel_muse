...
import { GitBranch, Activity, Play, AlertTriangle, PenTool, Clipboard, LayoutTemplate, Wand2, Info, X, CheckCircle, AlertCircle, History, RotateCcw, Zap, RefreshCw, Save, Sidebar, User, Globe, TrendingUp, MousePointer2, FileText, LayoutGrid } from 'lucide-react';
...
type TabMode = 'ANALYSIS' | 'WRITING' | 'OPTIMIZE' | 'RHYTHM';
...
const STRUCTURE_TEMPLATES = [
    ...
    { name: "网文黄金三章", content: "第一章：引入主角与冲突\n第二章：建立世界观与背景\n第三章：埋下伏笔与揭示秘密" }
];
...
export const PlotWeaver: React.FC<PlotWeaverProps> = ({ project, updateProject }) => {
  ...
  const handleAnalyzeRhythm = async () => {
      if (!plotOutline.trim()) {
          showToast("请先填写大纲内容", 'error');
          return;
      }
      setIsAnalyzingRhythm(true);
      try {
          const data = await analyzePlotRhythm(plotOutline);
          setRhythmData(data);
      } catch (e) {
          console.error(e);
          showToast("节奏分析失败", 'error');
      } finally {
  ...
  const handleWriteScene = async () => {
      // If text is selected, use that as the basis/context for the scene
      const contextText = selectedText || plotOutline;
      const promptText = scenePrompt || (selectedText ? "扩写这段情节为具体场景" : "扩写当前大纲");

      if (!promptText.trim()) return;
      setIsWriting(true);
      try {
          const result = await expandScene(project.premise, project.genre, contextText, promptText, project.characters, project.worldSettings, project.creativeSettings, project.echoes);
  ...
  const handleRewrite = async (prompt: string, label: string) => {
      ...
      setIsGeneratingPlot(true);
      try {
          // If text is selected, only rewrite that part
          if (selectedText && selectionRange) {
              ...
              updatePlotWithHistory(newContent, `${label} (局部)`);
            ...
            <div className="relative flex-1 flex flex-col overflow-hidden">
                        ...
                        {beats.length > 0 ? beats.map((beat, idx) => (
                            <div key={idx} className="bg-slate-900 border border-slate-700 rounded-lg p-4 hover:border-muse-500/50 transition-colors group">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <button 
                                        onClick={() => {
                                            setSelectedText(beat);
                                            setActiveTab('WRITING');
                                            setScenePrompt(`扩写这个情节节点：${beat.substring(0, 20)}...`);
                                        }}
                                        className="opacity-0 group-hover:opacity-100 text-xs bg-slate-800 text-muse-300 px-2 py-1 rounded hover:bg-muse-900 transition-all"
                                    >
                                        扩写此段
                                    </button>
                                </div>
                ...
                {selectedText && viewMode === 'TEXT' && (
                    <div className="absolute bottom-16 right-4 bg-slate-800 border border-muse-500/50 shadow-2xl rounded-lg p-2 flex flex-col gap-1 animate-fade-in z-30">
                        ...
                        <button 
                            onClick={() => { setActiveTab('WRITING'); setScenePrompt("基于这段大纲扩写场景"); }}
                            className="text-xs text-left px-3 py-2 hover:bg-slate-700 rounded text-slate-200 flex items-center gap-2"
                        >
                            <PenTool size={12} /> 扩写此段
                        </button>
                    </div>
                )}
      ...
      <div className="w-1/2 bg-slate-900 rounded-xl border border-slate-800 flex flex-col overflow-hidden relative">
        <div className="flex border-b border-slate-800 bg-slate-900/50">
            ...
            <button
                ...
                className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'RHYTHM' ? 'text-muse-400 border-b-2 border-muse-500 bg-muse-900/10' : 'text-slate-500 hover:text-slate-300'}`}
            >
                <TrendingUp size={16} /> 节奏视图
            </button>
            <button
                ...
                <div className="flex flex-col h-full animate-fade-in">
                    {rhythmData.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-slate-600">
                            ...
                            <button 
                                onClick={handleAnalyzeRhythm}
                                disabled={isAnalyzingRhythm || !plotOutline}
            ...
            {activeTab === 'WRITING' && (
                <div className="flex flex-col h-full space-y-4 animate-fade-in">
                    <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700 space-y-3 shrink-0">
                         ...
                         {selectedText && (
                            <div className="text-xs text-muse-400 mb-2 italic">
                                * 基于选中的 {selectedText.length} 字进行扩写
                            </div>
                         )}
                         <div className="flex gap-2">
                             <input 
                                ...
                                placeholder={selectedText ? "例如：扩写这段对话，增加动作描写..." : "例如：扩写第三幕中英雄与反派的对峙对话..."}
