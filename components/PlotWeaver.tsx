...
import { GitBranch, Activity, Play, AlertTriangle, PenTool, Clipboard, LayoutTemplate, Wand2, Info, X, CheckCircle, AlertCircle, History, RotateCcw, Zap, RefreshCw, Save, Sidebar, User, Globe, TrendingUp } from 'lucide-react';
...
export const PlotWeaver: React.FC<PlotWeaverProps> = ({ project, updateProject }) => {
  ...
  const handleWriteScene = async () => {
      ...
      setIsWriting(true);
      try {
          const result = await expandScene(project.premise, project.genre, plotOutline, scenePrompt, project.characters, project.worldSettings, project.creativeSettings, project.echoes);
