...
export const EntityDetailPanel: React.FC<EntityDetailPanelProps> = ({
  ...
  return (
    <div
      className={`w-1/3 bg-slate-900 rounded-xl border border-slate-800 flex flex-col overflow-hidden transition-all duration-300 ${targetEntity ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
    >
      {/* Entity Header */}
      <div className="p-6 border-b border-slate-800 bg-slate-950/20">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest ${targetEntity.type === 'CHARACTER' ? 'bg-indigo-900 text-indigo-300 shadow-lg' : 'bg-emerald-900 text-emerald-300 shadow-lg'}`}
          >
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="text-xs text-slate-500 flex items-center gap-3">
            <span className="flex items-center gap-1.5">
              <Brain size={14} className="text-amber-500" /> 已积累记忆:{' '}
        ...
        <div className="space-y-8 relative">
          ...
          {entityHistory.length === 0 ? (
            ...
            entityHistory.map((item, idx) => (
              <div
                key={item.id}
                className={`relative pl-8 transition-opacity ${item.status === 'ARCHIVED' ? 'opacity-40 hover:opacity-100' : ''}`}
              >
                <div
                  className={`absolute left-0 top-1 w-6 h-6 rounded-full border flex items-center justify-center z-10 shadow-lg transition-colors ${
