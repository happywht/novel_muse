...
export const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({
  ...
  const [edgeCreationDialog, setEdgeCreationDialog] = useState<{
    source: SimNode;
    target: SimNode;
  } | null>(null);
    ...
    <div className="flex-1 flex flex-col bg-[#0b1222] rounded-xl overflow-hidden border border-slate-800">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800 bg-slate-900/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500">
            {displayNodes.length}/{fullStats.nodes} 节点 · {displayEdges.length}/{fullStats.edges}{' '}
            关系
          </span>
        </div>
            ...
            <>
              <div className="flex items-center gap-1.5 bg-slate-950/50 px-2 py-1 rounded border border-slate-700/50">
                {Object.entries(GRAPH_LAYER_LABELS).map(([key, label]) => (
                  <label
                    key={key}
                    className="flex items-center gap-1.5 cursor-pointer px-1.5 hover:bg-slate-800 rounded transition-colors group"
                  >
                    <input
                      type="checkbox"
                    ...
                    />
                    <span
                      className={`text-[10px] font-bold ${activeLayers.includes(key) ? 'text-slate-200' : 'text-slate-500 group-hover:text-slate-400'}`}
                    >
                      {label}
                    </span>
                  </label>
      ...
      <div className="absolute bottom-4 left-4 flex flex-col gap-2 p-3 bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-lg pointer-events-none z-10 transition-opacity">
        ...
        {Object.entries(GRAPH_NODE_COLORS).map(([type, colors]) => (
          <div
            key={type}
            className={`flex items-center gap-2 transition-opacity ${activeLayers.includes(type) ? 'opacity-100' : 'opacity-30'}`}
          >
            <span
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="flex-1 text-center bg-slate-800 rounded py-2 px-3">
                <span className="text-xs text-slate-500 block mb-1">
                  {edgeCreationDialog.source.type}
                </span>
                <span className="text-sm font-bold text-slate-200">
                  {edgeCreationDialog.source.label}
                </span>
              </div>
              <div className="text-slate-500">→</div>
              <div className="flex-1 text-center bg-slate-800 rounded py-2 px-3">
                <span className="text-xs text-slate-500 block mb-1">
                  {edgeCreationDialog.target.type}
                </span>
                <span className="text-sm font-bold text-slate-200">
                  {edgeCreationDialog.target.label}
                </span>
              </div>
            </div>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    await createEdgeApi(
                      projectId,
                      edgeCreationDialog.source.id,
                      edgeCreationDialog.target.id,
                      relType
                    );
...
const NodeEditSidebar: React.FC<NodeEditSidebarProps> = ({
  ...
  const [description, setDescription] = useState(
    node.properties?.description || node.properties?.content || ''
  );
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 text-slate-300">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-800/50">
        ...
        <button
          onClick={onClose}
          className="p-1 text-slate-500 hover:text-white rounded-md hover:bg-slate-700 transition-colors"
        >
      ...
      <div className="flex-1 p-5 overflow-y-auto custom-scrollbar">
        {isEditing ? (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-1">
                详情 (Description/Content/Summary)
              </label>
              <textarea
                value={description}
          ...
          <div className="space-y-6">
            <div>
              ...
              {node.properties?.role && (
                <div className="text-xs font-medium text-muse-400 mb-4 bg-muse-400/10 inline-block px-2 py-1 rounded border border-muse-400/20">
                  {node.properties.role}
                </div>
              )}
              ...
              {node.properties?.order !== undefined && (
                <div className="text-xs font-medium text-muse-400 mb-4 bg-muse-400/10 inline-block px-2 py-1 rounded border border-muse-400/20">
                  排序: {node.properties.order}
                </div>
              )}
            </div>

            <div>
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 border-b border-slate-800 pb-1">
                详细描述
              </h4>
              <p className="text-sm font-serif leading-relaxed text-slate-400 whitespace-pre-wrap">
                {description || '暂无描述'}
              </p>
            </div>
          </div>
          ...
          (isEditing ? (
            <>
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="px-4 py-2 text-xs font-bold bg-muse-600 hover:bg-muse-500 text-white rounded shadow-lg flex items-center gap-2"
              >
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-bold bg-slate-700 hover:bg-slate-600 text-white rounded transition-colors w-full"
            >
              ✏️ 编辑节点
            </button>
          ))}
