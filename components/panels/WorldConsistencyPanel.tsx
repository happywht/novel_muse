...
export const WorldConsistencyPanel: React.FC<WorldConsistencyPanelProps> = ({
  ...
  const [activeCategory, setActiveCategory] = useState<
    'all' | 'SPATIAL_CONFLICT' | 'HIERARCHY_CYCLE' | 'LOGICAL_CONTRADICTION'
  >('all');
  ...
  const filteredIssues =
    activeCategory === 'all' ? issues : issues.filter((issue) => issue.type === activeCategory);
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-xs text-slate-500">世界观一致性检测帮助您发现设定中的矛盾和冲突</p>
          </div>
        ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-slate-400">{TYPE_LABELS[issue.type]}</span>
                      <span
