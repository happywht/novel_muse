...
export const CharacterArcSelector: React.FC<CharacterArcSelectorProps> = ({
  ...
  const [selectedType, setSelectedType] = useState<ArcType | undefined>(currentArc?.arcType);
  ...
  return (
    <div className="bg-slate-900/40 border border-slate-700/50 rounded-xl p-4 space-y-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-xs text-slate-500">帮助新手设计高质量角色成长路径</div>
      </div>
          ...
          <div className="space-y-3">
            {selectedTemplate.phases.map((phase) => {
              ...
              const phaseProgress = isCurrentPhase
                ? currentArc.phaseProgress
                : getPhaseIndex(phase, selectedTemplate) <
                    getPhaseIndex(currentArc.currentPhase, selectedTemplate)
                  ? 100
                  : 0;
                ...
                <div key={phase} className="border rounded-lg overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {isCurrentPhase && <span className="text-xs text-sky-400">当前阶段</span>}
                    </div>
                    ...
                    <div className="p-3 border-t border-slate-700/50">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-bold text-slate-200">{phaseProgress}%</span>
                        </div>
                        <input
                          type="range"
                          min="0"
