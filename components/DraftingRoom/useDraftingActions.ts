...
} from '../../services/validators/postWriteValidator';
...
import {
  fetchUnresolvedForeshadowing,
  fetchRelatedSubgraph,
  fetchPhysicalStatus,
  fetchFactions,
  simulatePropagation,
...
export const useDraftingActions = ({
  ...
  const [targetWordCount, setTargetWordCount] = React.useState<number>(
    CREATIVE_CONFIG.DEFAULT_TARGET_WORD_COUNT
  );
  ...
  const [postWriteViolations, setPostWriteViolations] = React.useState<
  ...
  const effectiveCreativeSettings = {
    ...
    referenceText:
      localReferenceText.trim() !== ''
        ? localReferenceText
        : project.creativeSettings.referenceText,
  };
      ...
      if (chapter) {
        ...
        if (chapter.expectedPOV) {
          const char = project.characters.find(
            (c) => c.name.includes(chapter.expectedPOV!) || chapter.expectedPOV!.includes(c.name)
          );
  ...
  React.useEffect(() => {
    if (
      ...
      targetWordCount === CREATIVE_CONFIG.DEFAULT_TARGET_WORDS.LITERARY
    )
      setTargetWordCount(CREATIVE_CONFIG.DEFAULT_TARGET_WORDS.WEB_NOVEL);
    else if (
      project.creativeSettings.promptProfile === 'LITERARY' &&
      targetWordCount === CREATIVE_CONFIG.DEFAULT_TARGET_WORDS.WEB_NOVEL
    )
      setTargetWordCount(CREATIVE_CONFIG.DEFAULT_TARGET_WORDS.LITERARY);
  ...
  const handleGenerateTwists = async () => {
    ...
    try {
      const context = project.chapters
        .slice(-3)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const rollingSummary =
        sortedChapters
          .slice(0, currentIndex >= 0 ? currentIndex : sortedChapters.length)
          .filter((c) => c.summary && c.summary.trim() !== '')
          .map((c) => `[第 ${c.order} 章: ${c.title}]\n${c.summary}`)
          .join('\n\n') || undefined;

      const povCharName = povCharId
      ...
      if (useBackend && useGraphContext) {
        ...
        try {
            ...
            forgeContext.characters.forEach((char) => {
              ...
              if (char.relationships && char.relationships.length > 0) {
                char.relationships.forEach((rel) => {
                  contextParts.push(
                    `  与 ${rel.targetName}: ${rel.type} (权重: ${rel.weight})${rel.trajectory ? ` [${rel.trajectory}]` : ''}`
                  );
                });
              }
            });
          ...
          if (forgeContext.locationContext) {
            contextParts.push('\n【场景设定】');
            contextParts.push(
              `[${forgeContext.locationContext.category}] ${forgeContext.locationContext.title}: ${forgeContext.locationContext.content}`
            );
          }
          ...
          if (forgeContext.plotContext) {
            contextParts.push('\n【情节背景】');
            const relatedChars =
              forgeContext.plotContext.relatedCharacters.length > 0
                ? ` (涉及: ${forgeContext.plotContext.relatedCharacters.join(', ')})`
                : '';
            contextParts.push(
              `${forgeContext.plotContext.title}${relatedChars}: ${forgeContext.plotContext.content}`
            );
          }
  ...
  const handleExtractEchoes = async () => {
    ...
    try {
      ...
      const changes = await analyzeStateChanges(
        generatedContent,
        chars,
        project.worldSettings || []
      );
  ...
  return {
    ...
    setLocalStyleTags,
    localReferenceText,
    setLocalReferenceText,

    // Phase 5
    factions,
    isFetchingFactions,
    handleFetchFactions: handleFetchFactionsLocal,
    showFactionPanel,
    setShowFactionPanel,
    showButterflyPanel,
