      ...
      {actions.postWriteViolations && actions.postWriteViolations.length > 0 && (
        <div className="bg-orange-900/20 border border-orange-500/40 p-4 rounded-xl space-y-2 mb-4 animate-in slide-in-from-top-2 duration-300">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => actions.clearValidationResults?.()}
          ...
          <div className="space-y-1.5 max-h-40 overflow-y-auto custom-scrollbar pr-1">
            {actions.postWriteViolations.map((v, i) => (
              <div
                key={i}
                className={`text-[10px] p-2 rounded border ${v.severity === 'error' ? 'bg-red-900/30 border-red-500/30 text-red-200' : 'bg-orange-900/20 border-orange-500/20 text-orange-200'}`}
              >
              ...
              >
                ...
                <div className="flex flex-wrap gap-1 mt-1 font-mono text-[8px] text-slate-500">
                  {insight.involvedEntities.map((e, i) => (
                    <span key={i} className="bg-slate-800 px-1 rounded">
                      {e}
                    </span>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {suggestedTwists.map((twist, idx) => (
                <div
                  ...
                  className={`text-[11px] p-2 rounded border cursor-pointer transition-all ${
                    activeTwist === twist
                      ? 'bg-muse-700/50 border-muse-400 text-white shadow-lg'
                      : 'bg-slate-900/50 border-slate-700 text-slate-400 hover:border-muse-600/50'
                  }`}
                >
                  {twist}
                </div>
                ...
                >
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] text-indigo-200">
                      <span className="text-indigo-500">HOOK:</span> {hook.subject} {hook.relation}{' '}
                      {hook.object}
                    </span>
                  </div>
                </div>
      ...
      <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
        ...
        <div className="flex flex-wrap gap-2">
          {(project.characters || []).length === 0 && (
            <p className="text-xs text-slate-500">暂无角色，请去灵魂熔炉创建。</p>
          )}
          ...
          <div className="flex">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium transition-all relative ${
                activeTab === 'params' ? 'text-white' : 'text-slate-400 hover:text-slate-300'
              }`}
            >
              <Sliders size={14} />
              参数
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium transition-all relative ${
                activeTab === 'reference' ? 'text-white' : 'text-slate-400 hover:text-slate-300'
              }`}
            >
              <FileText size={14} />
              参考
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium transition-all relative ${
                activeTab === 'prompt' ? 'text-white' : 'text-slate-400 hover:text-slate-300'
              }`}
            >
              <Sparkles size={14} />
              Prompt
              <span className="text-[8px] px-1 py-0.5 bg-purple-500/30 text-purple-300 rounded-full">
                New
              </span>
              {activeTab === 'prompt' && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-muse-400 rounded-full" />
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(project.worldSettings || []).length === 0 && (
                    <p className="text-xs text-slate-500">暂无设定，请去灵魂熔炉创建。</p>
                  )}
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="flex justify-between items-center gap-1">
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-medium rounded-lg flex flex-col items-center transition-all ${
                      pacing === 'SLOW_BURN'
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-medium rounded-lg flex flex-col items-center transition-all ${
                      pacing === 'BALANCED'
                        ? 'bg-muse-900/50 text-muse-300 border border-muse-500/30'
                        : 'bg-slate-900/50 text-slate-500 hover:bg-slate-700/50'
                    }`}
                  >
                    平衡推进
                      ...
                      (tag) => (
                        <button
                          ...
                          className={`text-[9px] px-2 py-0.5 rounded border transition-colors ${
                            actions.localStyleTags?.includes(tag)
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-1">
          ...
          {(project.drafts || []).map((draft) => (
            <div
              ...
              className={`p-2 rounded-lg border cursor-pointer group flex justify-between items-start transition-all ${
              ...
              <div className="flex-1 min-w-0">
                <h4
                  className={`text-xs font-medium truncate ${
                    activeDraftId === draft.id ? 'text-muse-300' : 'text-slate-300'
                  }`}
                >
                  {draft.title}
                </h4>
