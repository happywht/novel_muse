...
interface DraftEditorProps {
  ...
  onRewriteSelection: (
    ...
    contextAfter: string,
    resetSelection: (newText: string) => void
  ) => void;
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        ...
        emptyEditorClass:
          'cursor-text before:content-[attr(data-placeholder)] before:text-slate-600 before:font-serif before:italic before:absolute',
      }),
    ...
    editorProps: {
      attributes: {
        class:
          'prose prose-invert prose-lg w-full max-w-none font-serif leading-loose text-slate-300 min-h-[500px] outline-none',
      },
    },
  });
  ...
  return (
    <div className="relative h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 relative">
        ...
        {isProcessing && (
          <div className="absolute inset-0 bg-slate-900/40 cursor-not-allowed z-10 flex items-center justify-center"></div>
        )}
      </div>

      <div className="bg-slate-950/50 border-t border-slate-800 px-4 py-2 flex justify-end text-[10px] text-slate-500 font-mono">
        {editor.storage.characterCount.characters()} 字符 · {editor.storage.characterCount.words()}{' '}
        词
      </div>
    </div>
  );
