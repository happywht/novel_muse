...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  initialize: async () => {
    ...
    const localProjects =
      (await storageService.getItem<ProjectState[]>(STORAGE_KEYS.PROJECTS)) || [];
        ...
        if (list.length > 0) {
          ...
          if (localVersion && localVersion.lastModified > mostRecentRemote.lastModified) {
            ...
            const mergedChapters = (fullProject.chapters || []).map((remoteCh: any) => {
              ...
              const content =
                remoteCh.content === '' && localCh && localCh.content !== ''
                  ? localCh.content
                  : remoteCh.content;
              const beats =
                (!remoteCh.beats || remoteCh.beats.length === 0) && localCh?.beats
                  ? localCh.beats
                  : remoteCh.beats;
  ...
  switchProject: async (id) => {
    ...
    if (store.useBackend) {
      try {
        ...
        set({
          project: { ...INITIAL_PROJECT, ...fullProject },
          showProjectList: false,
          isLoading: false,
        });
        return;
  ...
  syncToBackend: async () => {
    ...
    const timerId = setTimeout(async () => {
      try {
        ...
        const hasComplexChanges = Object.keys(_internal.pendingPatch).some((key) =>
        ...
        if (hasComplexChanges) {
          ...
          console.log(
            '☁️ Incremental Sync (PATCH) to backend:',
            id,
            Object.keys(_internal.pendingPatch)
          );
  ...
  loadGlobalConfig: async () => {
    try {
      const savedConfig = await storageService.getItem<typeof DEFAULT_CONFIG>(
  ...
  updateGlobalConfig: async (updates: DeepPartial<typeof DEFAULT_CONFIG>) => {
    ...
    if (
      updates.performance?.cache?.enabled === false ||
      (updates.performance?.cache?.ttl &&
        updates.performance.cache.ttl !== currentConfig.performance.cache.ttl)
    ) {
    ...
    try {
      ...
      set({
        graphQuery: {
          ...
          loadingTraits: new Set(
            [...get().graphQuery.loadingTraits].filter((id) => id !== characterId)
          ),
        },
      });
    } catch (err) {
    ...
    try {
      ...
      set({
        graphQuery: {
          ...
          loadingEvolution: new Set(
            [...get().graphQuery.loadingEvolution].filter((id) => id !== characterId)
          ),
        },
      });
    } catch (err) {
    ...
    try {
      ...
      set({
        graphQuery: {
          ...
          loadingForeshadowing: new Set(
            [...get().graphQuery.loadingForeshadowing].filter((id) => id !== characterId)
          ),
        },
      });
    } catch (err) {
